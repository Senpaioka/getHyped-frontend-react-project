...
const HypedSection = () => {
  ...
  const LogoSVG = `
    <svg xmlns="http://www.w3.org/2000/svg" width="140" height="60" viewBox="0 0 208 84" fill="none">
  ...
  const handleMouseEnter = (e) => {
    // We stop propagation so entering the text doesn't 
    // trigger both the text pop and the section pop at once.
    e.stopPropagation();
    createPopup(e);
  };
  ...
  const createPopup = (e) => {
    ...
    popup.style.left = `${startX - 70}px`; 
    popup.style.top = `${startY - 30}px`;

    sectionRef.current.appendChild(popup);
  ...
  return (
    <section 
      ...
      className="relative bg-[#FAF7F2] pt-32 pb-40 overflow-hidden"
    >
